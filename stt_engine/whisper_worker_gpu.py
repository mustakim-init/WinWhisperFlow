    ...
    # DML path — onnxruntime with DirectML
        ...
        for line in sys.stdin:
            try:
                ...
                use_nst = float(request.get("no_speech_threshold")) if request.get("no_speech_threshold") is not None else no_speech_threshold
                use_lpt = float(request.get("log_prob_threshold")) if request.get("log_prob_threshold") is not None else log_prob_threshold
                ...
                text = trim_repetitive_text(text)
